...
```text
REPO: pds2225/text-to-todo-streamlit
BASE: master
```
...
4. `master` clean + ahead=0 + behind>0이면 `git merge --ff-only origin/master`만 허용.
5. dirty/ahead/diverged/다른 브랜치의 로컬 전용 변경은 보존. 삭제·덮어쓰기·자동 reset 금지.
6. `git reset --hard`, force push, `git clean -fd`, 임의 stash/drop 금지.
7. 필요하면 최신 `origin/master` 기준 별도 branch/worktree 생성. 안전 분리 불가 시 `BLOCKED`.
8. 이 `TASK.md`만 실행. 다른 레포 TASK/NEXT_TASK/옛 채팅 과업 금지.
9. 구현 후 테스트·commit·push. 명시 없으면 `master` 병합 금지.
...
## 입력검증
- 저장소/origin이 `pds2225/text-to-todo-streamlit`인지 확인하고 최신 원격 기준 확보.
## 빈상태
...
- 최신 `origin/master` 기준 작업 브랜치에서 구현→테스트→commit→push. 별도 지시 없으면 `master` 병합 금지.
## DONE/BLOCKED
